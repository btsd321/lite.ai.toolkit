...
Ort::Value YOLO12::transform(const cv::Mat &mat_rs)
{
    cv::Mat canvas;
...
void YOLO12::resize_unscale(const cv::Mat &mat, cv::Mat &mat_rs,
...
void YOLO12::detect(const cv::Mat &mat, std::vector<types::Boxf> &detected_boxes,
...
void YOLO12::generate_bboxes(const YOLO12ScaleParams &scale_params,
                             std::vector<types::Boxf> &bbox_collection,
                             std::vector<Ort::Value> &output_tensors,
                             float score_threshold, int img_height, int img_width)
{
    Ort::Value &pred = output_tensors.at(0);              // (1,n,84)
    auto pred_dims = output_node_dims.at(0);              // (1,n,84)
    ...
    {
        float *offset_obj_cls_ptr = (float *)pred.GetTensorData<float>() + (i * (num_classes + 4));
...
void YOLO12::nms(std::vector<types::Boxf> &input, std::vector<types::Boxf> &output,
