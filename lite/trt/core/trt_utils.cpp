...
void trtcv::utils::transform::create_tensor(
    const cv::Mat &mat, std::vector<float> &input_vector,
    std::vector<int64_t> input_node_dims, unsigned int data_format)
{
    ...
    if (mat.type() != CV_32FC(channels))
        ...
        mat_ref =
            mat;  // reference only. zero-time cost. support 1/2/3/... channels
    ...
    if (data_format == transform::CHW)
    {
        ...
        // Convert BGR to RGB while creating CHW tensor
        for (int c = 0; c < channels; ++c)
        {
            // Map BGR channels to RGB: B(0)->2, G(1)->1, R(2)->0
            int src_channel = (channels == 3) ? (2 - c) : c;
            for (int h = 0; h < rows; ++h)
            {
                for (int w = 0; w < cols; ++w)
                {
                    if (channels == 1)
                        input_vector[c * rows * cols + h * cols + w] =
                            mat_ref.at<float>(h, w);
                    else if (channels == 3)
                        input_vector[c * rows * cols + h * cols + w] =
                            mat_ref.at<cv::Vec3f>(h, w)[src_channel];
                    else
...
std::vector<float> trtcv::utils::transform::trt_load_from_bin(
...
void trtcv::utils::transform::trt_save_to_bin(const std::vector<float> &data,
                                              const std::string &filename)
{
...
void trtcv::utils::transform::trt_generate_latents(
    std::vector<float> &latents, int batch_size, int unet_channels,
    int latent_height, int latent_width, float init_noise_sigma)
{
    size_t total_size =
        batch_size * unet_channels * latent_height * latent_width;
...
void trtcv::utils::remove_small_connected_area(cv::Mat &alpha_pred,
                                               float threshold)
{
    cv::Mat gray, binary;
    ...
    int num_labels = cv::connectedComponentsWithStats(binary, labels, stats,
                                                      centroids, 8, 4);
