...
namespace trtcv {
    ...
    private:
        void letterbox(const cv::Mat &src, cv::Mat &out_image,
                       cv::Size &new_shape,
                       int stride = 32,
        ...
        void generate_bboxes(std::vector<types::Boxf> &bbox_collection, float *output,
                             float score_threshold, float scale, float pad_w, float pad_h);
