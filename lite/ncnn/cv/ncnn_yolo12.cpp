...
#include "ncnn_yolo12.h"
#include "lite/ncnn/core/ncnn_utils.h"
#include "lite/utils.h"

using ncnncv::NCNNYOLO12;

void NCNNYOLO12::transform(const cv::Mat &mat)
{
    ncnn::Mat input = ncnn::Mat::from_pixels(mat.data, ncnn::Mat::PIXEL_BGR2RGB, mat.cols, mat.rows);
    input.substract_mean_normalize(mean_vals, norm_vals);
    ncnn_in = input;
}

void NCNNYOLO12::resize_unscale(const cv::Mat &mat, cv::Mat &mat_rs,
...
void NCNNYOLO12::detect(const cv::Mat &mat, std::vector<types::Boxf> &detected_boxes,
                        ...
                        unsigned int topk, unsigned int nms_type)
{
    ...
    extractor.input(input_name, ncnn_in);
...
void NCNNYOLO12::generate_bboxes(const YOLO12ScaleParams &scale_params,
                                 std::vector<types::Boxf> &bbox_collection,
                                 ncnn::Extractor &extractor,
                                 float score_threshold, int img_height, int img_width)
{
    ncnn::Mat pred;
    extractor.extract(output_name, pred); // (1,n,84)

    const unsigned int num_anchors = pred.h;     // n
    const unsigned int num_classes = pred.w - 4; // 80
    ...
    {
        const float *offset_obj_cls_ptr = pred.row(i);
...
void NCNNYOLO12::nms(std::vector<types::Boxf> &input, std::vector<types::Boxf> &output,
